...
const vendorSchema = new mongoose.Schema({
  ...
  verification: {
    ...
    documents: [{
      ...
      // Per-document review state. admin-vendor.service#verifyVendorDocument
      // already wrote this field, but it was missing from the schema so
      // Mongoose (strict mode) silently dropped it on every save.
      status: {
        type: String,
  ...
  bankDetails: {
    ...
    /**
     * RazorpayX identities for gateway payouts. Created once per vendor on the first
     * payout and cached here: a contact and a fund account are permanent gateway
     * objects, so recreating them per payout would litter the account and burn
     * Razorpay's rate limit.
     *
     * Test-mode and live-mode ids are distinct objects at Razorpay, so both pairs are
     * kept and the resolved mode picks the right one.
     */
    payoutContactIds: {
      test: { type: String, default: '' },
      live: { type: String, default: '' }
    },
    payoutFundAccountIds: {
      test: { type: String, default: '' },
      live: { type: String, default: '' }
    }
  },
...
vendorSchema.methods.updatePerformanceMetrics = async function() {
  ...
  const reviewStats = await Review.aggregate([
    { $match: { vendor: this._id, 'moderation.status': 'approved' } },
    {
      $group: {
        _id: null,
  ...
  if (rentalStats.length > 0) {
    const { totalRentals, completedRentals, cancelledRentals, totalRevenue } = rentalStats[0];

    this.performance.metrics.totalRentals = totalRentals;
    this.performance.metrics.completedRentals = completedRentals;
    this.performance.metrics.cancelledRentals = cancelledRentals;
    this.performance.metrics.totalRevenue = totalRevenue;
    // Guard the divisions: `x / 0 || 0` evaluates to Infinity (which is truthy),
    // so a vendor with zero completed rentals used to persist Infinity here.
    this.performance.metrics.averageRentalValue =
      completedRentals ? totalRevenue / completedRentals : 0;
    this.performance.metrics.fulfillmentRate =
      totalRentals ? (completedRentals / totalRentals) * 100 : 0;
  }
...
vendorSchema.methods.canAcceptRental = async function() {
  ...
  if (this.subscription.limits) {
    // Rental.vendor / Product.vendor both reference the Vendor document,
    // not the owning User.
