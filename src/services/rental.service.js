...
class RentalService {
              ...
              items: [
                {
                  ...
                  inventory: inventoryItem._id,
                  quantity: 1,
                  // Copy product images into the delivery item so the
                  // partner dashboard can show them. Schema is [String] so
                  // store plain URLs.
  ...
  async getVendorRentals(vendorId, page = 1, limit = 10, filters = {}) {
    try {
      ...
      if (filters.status) {
        if (filters.status === 'completed') {
          matchStage.status = { $in: ['completed', 'delivered'] };
        } else {
  ...
  async confirmRental(rentalId, vendorId) {
    ...
    console.log("Confirming rental:", rentalId, "by vendor:", vendorId)
    ...
    try {
      ...
      // Auto-assign the best delivery person so the order appears on the
      // delivery partner dashboard immediately. Best-effort: if no eligible
      // partner exists (e.g. pincode/availability mismatch), the delivery
      // stays 'scheduled' and admin/vendor can assign it manually later.
      try {
        console.log("[DEBUG-AUTOASSIGN] 🔎 Finding delivery for rental:", rental._id, "| type: delivery | status: scheduled");
        ...
        if (!delivery) {
          console.log("[DEBUG-AUTOASSIGN] ❌ No scheduled delivery found for rental:", rental._id,
            "| check: type field must be 'delivery' AND status must be 'scheduled'");
        } else {
          console.log("[DEBUG-AUTOASSIGN] ✅ Delivery found:", delivery.deliveryNumber, "| id:", delivery._id);
          const DeliveryAIService = require("./delivery-ai.service");
          console.log("[DEBUG-AUTOASSIGN] 🚀 Calling autoAssignDelivery for delivery:", delivery._id);
          const assignResult = await DeliveryAIService.autoAssignDelivery(
            delivery._id,
            { assignedBy: vendorId },
          );
          console.log("[DEBUG-AUTOASSIGN] 📦 autoAssignDelivery result:", JSON.stringify(assignResult, null, 2));
          if (assignResult?.assigned) {
            logger.info(
              `Auto-assigned delivery ${delivery.deliveryNumber} to ${assignResult.assignedTo?.name || assignResult.assignedTo?._id} (score ${assignResult.score}%)`,
            );
          } else {
            logger.info(
              `Delivery ${delivery.deliveryNumber} not auto-assigned: ${assignResult?.reason || "no eligible partner"}`,
            );
          }
        }
      } catch (autoAssignError) {
        console.log("[DEBUG-AUTOASSIGN] 💥 autoAssignDelivery THREW:", autoAssignError.message);
        logger.error(
          `Auto-assign failed for rental ${rental._id}: ${autoAssignError.message}`,
        );
      }
  ...
  async cancelRental(rentalId, userId, userRole, reason) {
    ...
    try {
      ...
      // Cancel any scheduled/assigned deliveries for this rental
      const cancelledDeliveries = await Delivery.find({
        rental: rental._id,
        status: { $in: ['scheduled', 'assigned', 'batched', 'out_for_delivery', 'in_transit', 'reached'] },
      }).select('_id deliveryNumber status');
      if (cancelledDeliveries.length) {
        await Delivery.updateMany(
          { _id: { $in: cancelledDeliveries.map((d) => d._id) } },
      ...
      // Emit event (notifications + emails handled centrally in
      // rental.events.js RENTAL.CANCELLED handler; delivery status was
      // already set to cancelled above).
