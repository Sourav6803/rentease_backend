...
const getPropValueOrUndefinedIfIsSilent = (propName, data) => data.silent ? undefined : data[propName];
...
const buildAndroidNotification = data => {
  ...
  return Object.fromEntries(Object.entries(notification).filter(([, value]) => value !== undefined));
};
const buildAndroidMessage = (data, options) => {
  ...
  const fcmAndroidMessage = {
    ...
    ttl: extractTimeToLive(data) * 1000,
    // Convert seconds to milliseconds for FCM
  ...
  // Remove undefined values
  return Object.fromEntries(Object.entries(fcmAndroidMessage).filter(([, value]) => value !== undefined));
};
