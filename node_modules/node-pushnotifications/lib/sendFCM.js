...
const getRecipientList = obj => obj.tokens ?? [obj.token, obj.condition, obj.topic].filter(Boolean);
...
const sendFCM = (regIds, data, settings) => {
  ...
  const optionalProps = ['httpAgent', 'httpsAgent', 'projectId', 'databaseURL', 'storageBucket', 'serviceAccountId', 'databaseAuthVariableOverride'];
  ...
  if (containsValidRecipients(data)) {
    if (data.recipients.to) {
      Object.assign(data.recipients, {
        topic: data.recipients.to
      });
    }
